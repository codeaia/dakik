import React from 'react';
import {mount} from 'react-mounter';

import {mainLayout} from '../imports/ui/layouts/mainLayout.jsx';
import {authLayout} from '../imports/ui/layouts/authLayout.jsx';
import Nav from '../imports/ui/components/Nav.jsx';

import TagView from '../imports/ui/components/TagView.jsx';
import TagEdit from '../imports/ui/components/TagEdit.jsx';

import TaskNew from '../imports/ui/components/TaskNew.jsx';
import TaskDetail from '../imports/ui/components/TaskDetail.jsx';
import TaskEditContainer from '../imports/ui/components/TaskEdit.jsx';
import TaskViewContainer from '../imports/ui/components/TaskViewContainer.jsx';

import ProfileContainer from '../imports/ui/components/ProfileContainer.jsx';
import Settings from '../imports/ui/components/Settings.jsx';
import IntegrationAuth from '../imports/ui/components/IntegrationAuth.jsx';

import About from '../imports/ui/components/About.jsx';
import Auth from '../imports/ui/components/Auth.jsx';
import Statistics from '../imports/ui/components/Statistics.jsx';
import HomeContainer from '../imports/ui/components/Home.jsx';

import Err from '../imports/ui/components/Err.jsx';

FlowRouter.route('/', {
  name: 'home',
  action: function() {
    if (Meteor.userId()) {
      mount (mainLayout, {
        content: (<HomeContainer />),
        nav: (<Nav />)
      })
    } else {
		  FlowRouter.redirect('/auth');
    }
  }
});

FlowRouter.route('/tagEdit', {
  name: 'tagEdit',
  action: function() {
    if (Meteor.userId()) {
      mount (mainLayout, {
        content: (<TagEdit />)
      })
    } else {
		  FlowRouter.redirect('/auth');
    }
  }
});

FlowRouter.route('/tag/:tagName', {
  name: 'tagView',
  action: function(params) {
    if (Meteor.userId()) {
      mount (mainLayout, {
        nav: (<Nav />),
        content: (<TagView tagName={params.tagName} totalPomos='78' />)
      })
    } else {
		  FlowRouter.redirect('/auth');
    }
  }
});

FlowRouter.route('/tasks', {
  name: 'tasks',
  action: function() {
    if (Meteor.userId()) {
      mount (mainLayout, {
        content: (<TaskViewContainer />),
        nav: (<Nav />)
      })
    } else {
		  FlowRouter.redirect('/auth');
    }
  }
});

FlowRouter.route('/taskNew', {
  name: 'taskNew',
  action: function() {
    if (Meteor.userId()) {
      mount (authLayout, {
        nav: (<Nav />),
        content: (<TaskNew />)
      })
    } else {
		  FlowRouter.redirect('/auth');
    }
  }
});

FlowRouter.route('/taskEdit/:_id', {
  name: 'taskEdit',
  action: function(params, queryParams) {
    if (Meteor.userId()) {
      mount (authLayout, {
        nav: (<Nav />),
        content: (<TaskEditContainer id={params._id}/>)
      })
    } else {
		  FlowRouter.redirect('/auth');
    }
  }
});

FlowRouter.route('/auth', {
  name: 'auth',
  action: function(){
    if(Meteor.userId()) {
      FlowRouter.redirect('/');
    } else {
      mount (authLayout, {
        content: (<Auth />),
      })
    }
  }
});

FlowRouter.route('/statistics', {
  name: 'statistics',
  action: function() {
    if (Meteor.userId()) {
      mount (mainLayout, {
        nav: (<Nav />),
        content: (<Statistics />)
      })
    } else {
		  FlowRouter.redirect('/auth');
    }
  }
});

FlowRouter.route('/about', {
  name: 'about',
  action: function() {
    if (Meteor.userId()) {
      mount (mainLayout, {
        nav: (<Nav />),
        content: (<About />)
      })
    } else {
		  FlowRouter.redirect('/auth');
    }
  }
});

FlowRouter.route('/profile', {
  name: 'profile',
  action: function() {
    if (Meteor.userId()) {
      mount (mainLayout, {
        nav: (<Nav />),
        content: (<ProfileContainer />)
      })
    } else {
		  FlowRouter.redirect('/auth');
    }
  }
});

FlowRouter.route('/settings', {
  name: 'settings',
  action: function() {
    if (Meteor.userId()) {
      mount (mainLayout, {
        nav: (<Nav />),
        content: (<Settings />)
      })
    } else {
		  FlowRouter.redirect('/auth');
    }
  }
});

FlowRouter.route('/integrations', {
  name: 'integrations',
  action: function() {
    if (Meteor.userId()) {
      mount (mainLayout, {
        nav: (<Nav />),
        content: (<IntegrationAuth />)
      })
    } else {
		  FlowRouter.redirect('/auth');
    }
  }
});

FlowRouter.notFound = {
  	name: '404',
    action: function() {
  		mount (mainLayout, {
        content: (<Err />)
      })
    }
};
